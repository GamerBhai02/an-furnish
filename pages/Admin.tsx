  ...
  return (
    <div className="min-h-screen bg-stone-50 dark:bg-stone-900 pb-20">
                    ...
                    {leads.map(lead => (
                      <tr key={lead.id} className="hover:bg-stone-50 dark:hover:bg-stone-700/30 cursor-pointer" onClick={() => setSelectedLead(lead)}>
                        ...
                        <td className="px-6 py-4">
                          <div className="font-bold">{lead.contact.name}</div>
                ...
                {categories.map(cat => (
                  <div key={cat.id} className="group relative bg-white dark:bg-stone-800 rounded-xl overflow-hidden border border-stone-200 dark:border-stone-700">
                    ...
                    <div className="p-4 flex justify-between items-center">
                      ...
                      <button onClick={() => handleDeleteCategory(cat.id)} className="text-red-500 p-2 hover:bg-red-50 rounded-lg"><Trash2 size={18}/></button>
                    </div>
                  </div>
                ))}
             </div>
                ...
                {products.map(p => (
                  <div key={p.id} className="bg-white dark:bg-stone-800 rounded-xl overflow-hidden border border-stone-200 dark:border-stone-700 flex flex-col">
                    ...
                    <div className="p-4 border-t border-stone-100 dark:border-stone-700 flex justify-end">
                      <button onClick={() => handleDeleteProduct(p.id)} className="text-red-500 p-2 hover:bg-red-50 rounded-lg"><Trash2 size={18}/></button>
                    </div>
                  </div>
                ))}
             </div>
      ...
      {selectedLead && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-stone-800 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-stone-200 dark:border-stone-700 flex justify-between items-center sticky top-0 bg-white dark:bg-stone-800">
              <h3 className="text-xl font-bold">Order {selectedLead.orderId}</h3>
              <button onClick={() => setSelectedLead(null)}><X size={24}/></button>
            </div>
            <div className="p-6 space-y-6">
              ...
              <div>
                 ...
                 <pre className="text-xs bg-stone-100 dark:bg-stone-900 p-4 rounded-lg overflow-x-auto">
              ...
              <div className="border-t border-stone-200 dark:border-stone-700 pt-6">
                 ...
                 <div className="flex flex-wrap gap-2 mb-4">
                    {Object.values(OrderStatus).map(s => (
                      <button 
                        ...
                        className={`px-3 py-1 text-xs font-bold rounded-full border ${selectedLead.status === s ? 'bg-primary-600 text-white border-primary-600' : 'border-stone-300 dark:border-stone-600'}`}
                      >
                        {s}
                      </button>
                    ))}
                 ...
                 <div className="flex gap-2">
                   <input 
                      ...
                      className="flex-grow border rounded-lg p-2 dark:bg-stone-900 dark:border-stone-600"
                      value={noteInput}
      ...
      {isAddCategoryModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
           <div className="bg-white dark:bg-stone-800 p-6 rounded-xl w-full max-w-md animate-in zoom-in-95 duration-200">
              <h3 className="font-bold text-lg mb-4">Add Category</h3>
              <form onSubmit={handleAddCategory} className="space-y-3">
                 <input required className="w-full border p-2 rounded dark:bg-stone-700 dark:border-stone-600" placeholder="Name" value={newCategory.name} onChange={e => setNewCategory({...newCategory, name: e.target.value, slug: e.target.value.toLowerCase().replace(/\s+/g, '-')})} />
                 <input required className="w-full border p-2 rounded dark:bg-stone-700 dark:border-stone-600" placeholder="Slug" value={newCategory.slug} onChange={e => setNewCategory({...newCategory, slug: e.target.value})} />
                 <input className="w-full border p-2 rounded dark:bg-stone-700 dark:border-stone-600" placeholder="Image URL (optional)" value={newCategory.image} onChange={e => setNewCategory({...newCategory, image: e.target.value})} />
      ...
      {isAddProductModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
           <div className="bg-white dark:bg-stone-800 p-6 rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200">
              <h3 className="font-bold text-lg mb-4">Add Product</h3>
              <form onSubmit={handleAddProduct} className="space-y-3">
                 <input required className="w-full border p-2 rounded dark:bg-stone-700 dark:border-stone-600" placeholder="Title" value={newProduct.title} onChange={e => setNewProduct({...newProduct, title: e.target.value})} />
                 <div className="grid grid-cols-2 gap-2">
                   <select required className="w-full border p-2 rounded dark:bg-stone-700 dark:border-stone-600" value={newProduct.category} onChange={e => setNewProduct({...newProduct, category: e.target.value})}>
                      <option value="">Select Category</option>
                      {categories.map(c => <option key={c.id} value={c.slug}>{c.name}</option>)}
                   </select>
                   <input className="w-full border p-2 rounded dark:bg-stone-700 dark:border-stone-600" placeholder="Price Range" value={newProduct.priceRange} onChange={e => setNewProduct({...newProduct, priceRange: e.target.value})} />
                 </div>
                 <input className="w-full border p-2 rounded dark:bg-stone-700 dark:border-stone-600" placeholder="Tagline" value={newProduct.tagline} onChange={e => setNewProduct({...newProduct, tagline: e.target.value})} />
                 <textarea className="w-full border p-2 rounded dark:bg-stone-700 dark:border-stone-600" placeholder="Description" value={newProduct.description} onChange={e => setNewProduct({...newProduct, description: e.target.value})} />
                 <input className="w-full border p-2 rounded dark:bg-stone-700 dark:border-stone-600" placeholder="Image URL" value={newProduct.image} onChange={e => setNewProduct({...newProduct, image: e.target.value})} />
                 <div className="flex justify-end gap-2 mt-4">
